            ...
            if (config.apis) {
                ...
                for (const apiConfig of apis) {
                    ...
                    if (apiConfig.type === "file") {
                        ...
                        code.push("configuredApiPlugins.push(openApiPlugin({", `  type: "file",`, `  input: ${JSON.stringify(Object.fromEntries(schemas.map((s) => [s.version, s.inputPath])))},`, `  path: ${JSON.stringify(apiConfig.path)},`, `  tagPages: ${JSON.stringify(tags)},`, `  options: {`, `    examplesLanguage: config.defaults?.apis?.examplesLanguage ?? config.defaults?.examplesLanguage,`, `    disablePlayground: config.defaults?.apis?.disablePlayground,`, `    disableSidecar: config.defaults?.apis?.disableSidecar,`, `    showVersionSelect: config.defaults?.apis?.showVersionSelect ?? "if-available",`, `    expandAllTags: config.defaults?.apis?.expandAllTags ?? true,`, `    expandApiInformation: config.defaults?.apis?.expandApiInformation ?? false,`, `    transformExamples: config.defaults?.apis?.transformExamples,`, `    ...(apis[${apiIndex}].options ?? {}),`, `  },`, `  schemaImports: {`, ...schemaMapEntries.map(([key, processed]) => `    "${key.replace(/\\/g, "\\\\")}": () => import("${processed.filePath.replace(/\\/g, "/")}?d=${processed.processedTime}"),`), `  },`, "}));");
                    }
                    else {
