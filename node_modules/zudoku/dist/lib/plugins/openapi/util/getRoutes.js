...
import { generatePath, Navigate, redirect, useLocation, useParams, } from "react-router";
...
const NonTagPagesOperationList = ({ render, path, }) => {
...
const createNonTagPagesRoute = ({ path }) => ({
    path,
    async lazy() {
        const { OperationList } = await import("../OperationList.js");
        return {
            element: (_jsx(NonTagPagesOperationList, { path: path, render: (tag) => _jsx(OperationList, { tag: tag }) })),
        };
    },
});
