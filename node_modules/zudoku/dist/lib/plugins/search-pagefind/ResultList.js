...
export const ResultList = ({ basePath, searchResults, searchTerm, onClose, maxSubResults = 4, }) => {
    ...
    return (_jsxs(CommandList, { className: "max-h-[450px]", ref: commandListRef, children: [searchTerm && searchResults.length > 0 && (_jsx(CommandGroup, { className: "text-sm text-muted-foreground", heading: `${searchResults.length} results for "${searchTerm}"` })), searchTerm &&
                searchResults.map((result) => (_jsxs(CommandGroup, { children: [_jsx(CommandItem, { asChild: true, value: `${result.meta.title}-${result.url}`, className: hoverClassname, onSelect: () => {
                                ...
                                onClose();
                            }, children: _jsxs(Link, { to: stripBasePath(result.url), children: [result.meta.section === "openapi" ? (_jsx(BracketsIcon, {})) : (_jsx(FileTextIcon, {})), result.meta.title] }) }), result.sub_results
                            .sort(sortSubResults)
                            ...
                            .map((subResult) => (_jsx(CommandItem, { asChild: true, value: `sub-${result.meta.title}-${subResult.url}`, className: hoverClassname, onSelect: () => {
                                ...
                                onClose();
                            }, children: _jsx(Link, { to: stripBasePath(subResult.url), onClick: onClose, children: _jsxs("div", { className: "flex flex-col items-start gap-2 ms-2.5 ps-5 border-l border-muted-foreground/50", children: [_jsx("span", { className: "font-bold", children: subResult.title }), _jsx("span", { className: "text-[13px] [&_mark]:bg-primary [&_mark]:text-primary-foreground", 
                                            // biome-ignore lint/security/noDangerouslySetInnerHtml: Pagefind provides sanitized HTML
                                            dangerouslySetInnerHTML: { __html: subResult.excerpt } })] }) }) }, `sub-${result.meta.title}-${subResult.url}`)))] }, [result.meta.title ?? result.excerpt, result.url].join("-"))))] }));
};
